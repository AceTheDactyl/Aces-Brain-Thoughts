...
The APL Core Token Set provides the foundational grammar layer upon which all domain-specific extensions are built. Version 0.9 defines the complete \textbf{288-token universe} across four categories: identity tokens, meta-operators, domain selectors, and safety tokens.

\subsection{288-Token Universe Breakdown}

The core token universe consists of four categories:
...
\midrule
Identity Tokens & 162 & Self-referential: \verb|Φ:U(U)TRUE@1| \\
Meta-Operators & 54 & Process control: stabilize, propagate, integrate, modulate, resolve, collapse \\
Domain Selectors & 54 & Domain activation: geometry, dynamics, chemistry, deepphysics, biology, celestial \\
Safety Tokens & 30 & Runtime protection: safe, warn, danger, block, paradox \\
\midrule
\textbf{Core Total} & \textbf{288} & (some overlap in enumeration) \\
\bottomrule
\end{tabular}
\end{table}

Additionally, the primitives layer provides:
...
\subsection{Tier Permissions}

Machines are unlocked progressively across tiers:

\begin{table}[h]
\centering
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Tier} & \textbf{Allowed Machines} & \textbf{Constraints} \\
\midrule
1 & U, D & Tier1 cannot be rewritten \\
2 & U, D, M, E, C & Requires Tier1 coherence \\
3 & U, D, M, E, C, Mod & All operators + safety \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Spiral Bindings}

Each field has a default machine affinity:

\begin{itemize}[leftmargin=2em]
    \item $\Phi \to D$ — Structure tends to integrate (Down)
    \item $e \to U$ — Energy tends to project (Up)
    \item $\pi \to M$ — Emergence tends to modulate (Middle)
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    \item \textbf{Coherence} $\geq$ 0.60 required for tier advancement
    \item \textbf{Load} $\leq$ 0.80 prevents runaway
    \item \textbf{Recursion} $\leq$ 3 prevents infinite loops
    \item \textbf{PARADOX\_TERMINAL:} No exit without external reset
    \item \textbf{No constant mutation:} Tier1 tokens are immutable
\end{itemize}

\subsection{UMOL Principle}

\textit{No perfect modulation; residue always remains.}

Every M (modulation) operation leaves an UNTRUE shadow:
\[
    M(x) \to \text{TRUE} + \varepsilon(\text{UNTRUE}) \quad \text{where } \varepsilon > 0
\]
...
\midrule
\multicolumn{2}{l}{\textit{288-Token Universe:}} \\
Identity Tokens & 162 \\
Meta-Operators (Tier-2) & 54 \\
Domain Selectors (Tier-3) & 54 \\
Safety Tokens & 30 \\
\midrule
\textbf{Core 288 Total} & \textbf{288} \\
\midrule
\multicolumn{2}{l}{\textit{Primitives:}} \\
Tri-Spiral Tokens & 6 \\
Cross-Spiral Tokens & 54 \\
\midrule
\textbf{Primitives Total} & \textbf{60} \\
\midrule
\multicolumn{2}{l}{\textit{Domain Extensions:}} \\
Tier-3 Chemical & 972 \\
Tier-3 Biology & 972 \\
Tier-3 Celestial & 972 \\
\midrule
\textbf{Domain Total} & \textbf{2,916} \\
\midrule
\textbf{Grand Total} & \textbf{3,264} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Version 1.5} (2025-11-25)
\begin{itemize}[leftmargin=2em]
    \item Expanded Core Token Set to full 288-token universe
    \item Identity tokens (162): Self-referential machine operators
    \item Meta-operators (54): stabilize, propagate, integrate, modulate, resolve, collapse
    \item Domain selectors (54): geometry, dynamics, chemistry, deepphysics, biology, celestial
    \item Safety tokens (30): safe, warn, danger, block, paradox
    \item Tier permissions: progressive machine unlocking across tiers
    \item Spiral bindings: Φ→D, e→U, π→M default affinities
    \item UMOL principle: No perfect modulation; residue always remains
    \item Safety constraints: coherence $\geq$ 0.60, load $\leq$ 0.80, recursion $\leq$ 3
    \item Grand total: 288 core + 60 primitives + 2,916 domain = 3,264 tokens
\end{itemize}
...
\begin{itemize}[leftmargin=2em]
    ...
    \item Initial 222-token framework
\end{itemize}
